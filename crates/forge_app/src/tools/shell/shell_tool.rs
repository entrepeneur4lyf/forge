...
use anyhow::bail;
use forge_domain::{Environment, ExecutableTool, Executor, NamedTool, ToolDescription, ToolName, ToolOutput};
...
pub struct ShellInput {
    ...
    pub cwd: PathBuf,
    /// Is commend expecting an input.
    #[serde(default)]
    pub is_expecting_input: bool,
}
...
impl ExecutableTool for Shell {
    ...
    async fn call(&self, input: Self::Input, _: Option<&mut Executor>) -> anyhow::Result<ToolOutput> {
        ...
        if input.is_expecting_input {
            ...
            let output = CommandExecutor::new(command)
                .colored()
                .execute()
                .await?;
...
mod tests {
    ...
    async fn test_shell_empty_command() {
        let shell = Shell::new(test_env());
        let result = shell
            .call(ShellInput { command: "".to_string(), cwd: env::current_dir().unwrap(), is_expecting_input: false }, None)
            .await;
        assert!(result.is_err());
        assert_eq!(
    ...
    async fn test_shell_full_path_command() {
        ...
        let result = shell
            .call(ShellInput { command: cmd.to_string(), cwd: env::current_dir().unwrap(), is_expecting_input: false }, None)
            .await;
